...
1. The coordinator acquires the entity row lock and an entity-scoped advisory transaction lock. It pins the desired definition version, active publication revision, stewardship decision epoch, private graph generation, expected graph digest, member contract generations and digests, and semantic engine versions.
...
`mdm.create()` validates and stores a complete immutable definition. Validation checks the source relation types, entity execution role, grants, row-security behavior, source-key constraints, mapped column types, built-in cleaner and comparator options, authoritative field usage, candidate-plan completeness, public output compatibility, golden-policy inputs, and the negotiated `external_graph_refresh` capability. It compiles the canonical private graph, creates every member transactionally with `EXTERNAL` orchestration and initialization disabled, and stores the member contracts and graph digest. It does not change the active public entity until `mdm.refresh()` successfully activates the desired version.

The operation stores the user definition, expanded definition, semantic digest, creator, execution role, comment, parent version, compiled logical candidate plan, private graph digest and member contracts, and semantic version manifest. If graph creation or validation fails, the new version is not left partially installed.

### 13.2 `mdm.describe()`

`mdm.describe()` explains both the user model and the operational state. It shows the concise and fully expanded definitions, active and desired versions, public schema compatibility, source modes and boundaries, change and deletion completeness, execution role, fields and masking policies, match roles and evidence groups, the logical candidate plan and hard limits, private graph health, graph digest, member contract generations and digests, active publication revision, earliest retained explanation revision, pending definition or stewardship work, entity and review counts, last successful refresh, and any blocking error.
